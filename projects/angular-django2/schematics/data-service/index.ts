 ...
 */
function getNames(resourceName: string) {
...
function generateServiceContent(
  ...
  names: ReturnType<typeof getNames>,
): string {
...
export function dataService(options: DataServiceSchema): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    const serviceContent = generateServiceContent(options, names);

    if (tree.exists(serviceFilePath)) {
      context.logger.warn(`Service file already exists at ${serviceFilePath}. Skipping...`);
    } else {
      tree.create(serviceFilePath, serviceContent);
      context.logger.info(`✓ Created ${serviceFilePath}`);
    }

    // Generate spec file if not skipped
    if (!options.skipTests) {
      const specFilePath = normalize(
        path.join(destinationPath, `${names.fileName}.data.service.spec.ts`),
      );
      const specContent = generateSpecContent(options, names);

      if (tree.exists(specFilePath)) {
        context.logger.warn(`Spec file already exists at ${specFilePath}. Skipping...`);
      } else {
        tree.create(specFilePath, specContent);
        context.logger.info(`✓ Created ${specFilePath}`);
      }
    }

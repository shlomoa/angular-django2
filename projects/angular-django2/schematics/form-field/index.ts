...
function resolveOptions(tree: Tree, options: FormFieldSchema): ResolvedFormFieldOptions {
  assertSupportedOptions(options);
  if (!/^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/.test(options.name)) {
    throw new SchematicsException('The form-field name must be non-empty kebab-case.');
  }
  ...
  assertPackageDependencies(tree, 'form-field', [
    '@angular/forms',
    '@angular/material',
    '@angular/cdk',
  ]);

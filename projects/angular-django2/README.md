...
ng generate angular-django2:page orders --project=my-app --path=src/app/features/orders --routePath=orders
...
- `page`: generates a standalone `OnPush` Angular Material feature page, its
  ...
  - Options: `--path`, `--project`, `--routePath`, `--access=public|protected`,
    `--authGuard`, `--navigationLabel`, and `--navigationIcon`

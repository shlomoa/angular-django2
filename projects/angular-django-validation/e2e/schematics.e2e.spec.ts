 ...
 * Prefers a packed .tgz tarball if present, falling back to the dist directory.
 */
function getLibraryPackagePath(): string {
  const repoRoot = getRepoRoot();
  const distDir = path.join(repoRoot, 'projects', 'angular-django2', 'dist');
  const tarball = fs.existsSync(distDir)
    ? fs.readdirSync(distDir).find((f) => f.startsWith('angular-django2-') && f.endsWith('.tgz'))
    : null;

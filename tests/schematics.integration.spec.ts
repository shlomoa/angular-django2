...
`);
    ...
    it('INT-WS-02: generates lint workspace files (eslint config, scripts, deps, architect target)', async () => {
      ...
      const tree = await runner.runSchematic('workspace-setup', { name: 'demo-app' }, appTree);
  ...
  describe('openapi-setup schematic integration', () => {
    ...
    it('INT-API-02: handles custom paths correctly', async () => {
      const tree = await runner.runSchematic(
        'openapi-setup',
        {
          openapiSpecFile: 'custom/schema.json',
...
`,
  ...
  describe('material-app schematic integration', () => {
    beforeEach(() => {
      appTree.create(
        '/package.json',
        JSON.stringify({
    ...
    function createMinimalProjectStructure(tree: UnitTestTree, projectName: string): void {
      ...
      // Create minimal app files that material-app schematic expects
      tree.create(
...
`,
    ...
    it('INT-APP-01: generates complete application with Material setup', async () => {
      ...
      // The material-app schematic calls externalSchematic for application generation,

...
### 2.5 Observed schematic input options on `main` (to be verified in Phase 2)

From each `schema.json` on `main` (`7e7047a`). This is **observation, not
evidence**; Phase 2 confirms behavior with code and tests.

| Schematic           | `--document` | `--nodeId` | Other OpenUI option | Candidate evidence                                                                                   |
| :------------------ | :----------- | :--------- | :------------------ | :--------------------------------------------------------------------------------------------------- |
| `reactive-form`     | yes          | yes        | —                   | `schematics.reactive-form.spec.ts` `TC-REACTIVE-FORM-OPENUI-01…06`                                   |
| `form-field`        | yes          | yes        | —                   | `schematics.form-field.spec.ts` `TC-FORM-FIELD-OPENUI-01…03`                                         |
| `field-component`   | yes          | yes        | —                   | `schematics.field-component.spec.ts` `TC-FIELD-OPENUI-01`                                            |
| `component`         | yes          | yes        | —                   | `schematics.composition.spec.ts` `TC-COMPOSE-*`                                                      |
| `complex-component` | yes          | yes        | —                   | `schematics.composition.spec.ts` `TC-COMPOSE-*`                                                      |
| `embed-component`   | no           | no         | `--slot`            | `schematics.composition.spec.ts` `TC-COMPOSE-*`                                                      |
| `page`              | yes          | yes        | —                   | `schematics.openui-app.spec.ts` `TC-APP-*`                                                           |
| `application`       | yes          | yes        | —                   | `schematics.openui-app.spec.ts` `TC-APP-*`                                                           |
| `material-app`      | yes          | yes        | —                   | `schematics.openui-app.spec.ts` `TC-APP-*`                                                           |
| `workspace-setup`   | yes          | no         | —                   | `schematics.openui-app.spec.ts` `TC-APP-*`                                                           |
| `data-service`      | yes          | yes        | —                   | `schematics.openui-app.spec.ts` `TC-APP-*`                                                           |
| `material-setup`    | no           | no         | —                   | CLI-driven by design (#127, `f833a81`)                                                               |
| `app-shell`         | no           | no         | —                   | CLI-driven by design (#127, `f833a81`); OpenUI `ShellPage` maps to `material-app` layout (`a11e9f3`) |
| Master compiler     | n/a          | n/a        | —                   | Validation-only: `unit/integration/openui-application.integration.spec.ts`                           |

Baseline on `main` (`7e7047a`), run in a Linux clone: `npm run build` ✅,
`npm run test:node` ✅ 24 files / 243 tests.

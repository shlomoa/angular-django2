...
When `--primitives-path` contains the canonical
[`form-field`](form-field.md) output for a field, the form composes it instead
of inlining a Material control. Resolution uses private schematic metadata from
the `form-field` naming and option contract; it never parses generated
TypeScript. The canonical path is
`<primitives-path>/<field>-field/<field>-field.ts`.

`field-component` is the supported narrow convenience façade over the same
canonical generation path. Its output therefore resolves through the canonical
descriptor rather than becoming a competing primitive. The descriptor supplies
the component class, selector, control kinds, and exactly these bindings:
`fieldId`, `label`, `controlType`, `hint`, `placeholder`, `required`, and
`serverErrors`. Fields without that canonical output explicitly fall back to an
inline `mat-form-field` control.

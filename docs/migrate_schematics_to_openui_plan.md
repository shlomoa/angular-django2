...
| **`app-shell`**                                       | `--project`                                                                                 | CLI project identifier                                              | — (SSR/prerender build tooling; `shellPage` is `material-app`'s layout)                                | **Infrastructure Tooling**: Stays CLI-driven (Angular pass-through)                                             |
...
- **Phase 7 implementation notes** (resolved low-ambiguity decisions):
  ...
  - The mapping doc records the actual AST-input coverage instead of "100%": every schematic converted by phases 2–4 compiles from OpenUI, while `app-shell` and standalone `material-setup` are CLI-driven by design: `app-shell` is a pass-through to Angular's SSR app-shell generator with no OpenUI counterpart (`ShellPage` is `material-app`'s layout), and `material-setup`'s options are exactly the `Presentation` tokens, which `material-app --document` already reads and passes on. Widgets (`Table`, `Dialog`, …) are listed as not yet available and belong to `openui-spec-implementation-plan.md`.
  ...
  - Mapping correction (maintainer decision): OpenUI `ShellPage` maps to `material-app`'s layout, not to `app-shell`. The `app-shell` command keeps its name, because it passes through to Angular's own `@schematics/angular:app-shell`; it is listed as SSR/prerender tooling with no OpenUI counterpart.
